 ...
 * The schema for these attributes lives in the package.json of the hasflow extension.
...
export class HasflowDebugSession extends LoggingDebugSession {
	...
	public constructor(fileAccessor: FileAccessor) {
		...
		this._runtime = new HasflowRuntime(fileAccessor);

		// setup event handlers
		this._runtime.on('stopOnEntry', () => {
			this.sendEvent(new StoppedEvent('entry', HasflowDebugSession.threadID));
		});
		this._runtime.on('stopOnStep', () => {
			this.sendEvent(new StoppedEvent('step', HasflowDebugSession.threadID));
		});
		this._runtime.on('stopOnBreakpoint', () => {
			this.sendEvent(new StoppedEvent('breakpoint', HasflowDebugSession.threadID));
		});
		this._runtime.on('stopOnDataBreakpoint', () => {
			this.sendEvent(new StoppedEvent('data breakpoint', HasflowDebugSession.threadID));
		});
		this._runtime.on('stopOnInstructionBreakpoint', () => {
			this.sendEvent(new StoppedEvent('instruction breakpoint', HasflowDebugSession.threadID));
		});
		this._runtime.on('stopOnException', (exception) => {
			if (exception) {
				this.sendEvent(new StoppedEvent(`exception(${exception})`, HasflowDebugSession.threadID));
			} else {
				this.sendEvent(new StoppedEvent('exception', HasflowDebugSession.threadID));
			}
		});
	...
	protected async launchRequest(response: DebugProtocol.LaunchResponse, args: ILaunchRequestArguments) {
		...
		var env : NodeJS.Dict<string>;
		if (args.env != undefined) {
			env = {}
			Object.keys(args.env).forEach(
				(key) => {
  ...
  protected RestartRequest(response: DebugProtocol.RestartResponse, args: DebugProtocol.RestartArguments, request?: DebugProtocol.Request): void {
	...
	protected threadsRequest(response: DebugProtocol.ThreadsResponse): void {
		...
		response.body = {
			threads: [
				new Thread(HasflowDebugSession.threadID, "thread 1")
			]
		};
		this.sendResponse(response);
	}
