...
/**
 * A Hasflow runtime with minimal debugger functionality.
 */
export class HasflowRuntime extends EventEmitter {
	...
	public async start(program: string, env: NodeJS.Dict<string>, stopOnEntry: boolean, debug: boolean): Promise<void> {
		...
		return new Promise(async (resolve, reject) => {
			...
			this.debugProcess.on('error', (error) => {
				this.sendEvent('stopOnException', 'Process error: ' + error.message);
				reject(error);
			});
	...
	/**
	 * "Step into" for Hasflow debug means: go to next character
	 */
	...
	/**
	 * "Step out" for Hasflow debug means: go to previous character
	 */
	public stepOut() {
		if (typeof this.currentColumn === 'number') {
			this.currentColumn -= 1;
	...
	private getLine(line?: number): string {
		return "This is not actually the line!";//this.sourceLines[line === undefined ? this.currentLine : line].trim();
	}
